...
while P_error(end)>epsilon
    ...
    P_error(end+1) = norm(P_est{end}-P_est{end-1});
...
% BPA = B'*PP*A;
% BPB = B'*PP*B;
% adp_est_error = norm(APA-APA_est) + norm(BPA-BPA_est) + norm(BPB-BPB_est);
...
ctmp = zeros(nbar,1);
for i=1:nbar
    ctmp(i) = Bc(i)^2/(Ac(i,:)*inv(P_hat)*Ac(i,:)');
end
c = min(ctmp);

% MPC horizon
N = t;
F = P_hat;   % the final penalty matrix
while 1
    cvx_begin;
